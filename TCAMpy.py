...
class TModel:
    ...
    def create_dashboard(self):
        ...
        with col1:
            ...
            self.side_length = st.slider("Side Length (10um)", 10, 200, value=self.side_length)
            self.cycles      = st.slider("Model Duration (hours)", 50, 1000, value=self.cycles)
            self.pmax        = st.slider("Max Proliferation Potential", 1, 20, value=self.pmax)
            self.PA          = st.slider("Apoptosis Chance (RTC) (%)", 0, 100, value=self.PA)
            self.CCT         = st.slider("Cell Cycle Time (hours)", 1, 48, value=self.CCT)
            self.Dt          = st.slider("Time Step (days)", 0.01, 1.0, value=self.Dt, step=0.01)
            self.PS          = st.slider("STC-STC Division Chance (%)", 0, 100, value=self.PS)
            self.mu          = st.slider("Migration Capacity", 0, 10, value=self.mu)
            self.PP          = int(self.CCT * self.Dt / 24 * 100)
            self.PM          = 100 * self.mu / 24

            # Define current init configuration
            init_config = (self.side_length, self.cycles, self.pmax, self.PA, self.CCT, self.Dt, self.PS, self.mu)
            ...
            x_coord    = st.number_input("X Coordinate", 0, self.side_length-1, value=self.side_length//2)
            y_coord    = st.number_input("Y Coordinate", 0, self.side_length-1, value=self.side_length//2)
            cell_value = st.number_input("Cell Value", 0, self.pmax+1, value=self.pmax+1)
        ...
        with col3:
            if "model_result" in st.session_state:
                ...
                if "stats_df" in st.session_state:
                    ...
                    axs[0].plot(np.linspace(0, self.cycles - 1, 11, dtype=int), df[stc_cols].mean(), 'C1', label='STC')
                    axs[0].plot(np.linspace(0, self.cycles - 1, 11, dtype=int), df[rtc_cols].mean(), 'C2', label='RTC')
                    axs[0].set_title("Average Tumor Cell Count")
                    axs[0].set_xlabel("Model Time (hours)")
                    axs[0].set_ylabel("Number of Cells")
                    axs[0].legend()

                    axs[1].bar(pp_cols, df[pp_cols].mean(), edgecolor='black')
                    axs[1].set_title("Average Proliferation Potential Distribution")
                    axs[1].set_xlabel("Proliferation Potential")
