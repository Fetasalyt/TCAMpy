...
of the model. See :ref:`api_docs` for further details about them, or the pdf files in the repository (https://github.com/RRichardB/TCAMpy/tree/main/docs). After you've created the class with the parameters of your
...
This function automatically runs a single simulation. If 'plot' is set to True, the output includes multiple plots:
a heatmap of the final colony, cell number, area, and radius line charts, a histogram of the final proliferation potentails, a mutation map, and a tumor boundary map.
If the 'stats' parameter is set to True, various statistical properties of the model will be printed about the field and the cells. If 'animate' is set to True, the
function returns an animation of the growth. Note that animation does not work while using 'inline' backend. You must change the backend first.

You can also modifiy the initial state by calling the following functions before running the model:

.. code-block:: python

    M.mod_cell(x, y, value)   # OR: M.cancer[y][x] = value
...
You can change the initial state as much as you'd like before running the model. You can remove the automatically created STC by changing it's value to 0.
(You don't need to call this function if you don't want to modifiy the initial state, running the model creates a basic initial state, if you didn't define one before.)

You don't have to use this function to define a unique initial state; you can just make 'M.cancer' equal to a predefined numpy array or simply change it's value at a certain coordinate.
Be careful to not put cells to the edge of the area. Here is a complete example of a custom initial state, where the diagonal is filled with STCs (except on the edge).
...
like to use for them as a numpy array. (If you want to stick with the default initial state just say 'M.cancer' without modifying it or give an empty numpy array.)
You can change 'M.cancer' or give a completely different numpy array as a parameter, if you want a different initial state. This function returns the results
